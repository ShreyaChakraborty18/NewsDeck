...
const News = (props) => {
    ...
    return (
        <>
            <h2 className='text-center' style={{ margin: "35px 0px", marginTop: '90px' }}>NewsDeck - Top {capitalizeFirstLetter(props.category)} Headlines</h2>
            {loading && <Spinner />}
                        ...
                        {articles.map((element) => {
                            return <div className="col-md-4" key={element.url}>
                                <NewsItem title={element.title ? element.title : ""} description={element.description ? element.description : ""}
                                    imageUrl={element.urlToImage} newsUrl={element.url} author={element.author ? element.author : "Unknown"} date={new Date(element.publishedAt).toUTCString()} source={element.source.name} />
                            </div>
                        })}
                    </div>
